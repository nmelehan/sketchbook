...
void addText(RPoint[] points, color swarmColor) {
  final HSwarm swarm = new HSwarm().speed(.5).turnEase(0.03).twitch(10);

  final RPoint[] textPoints = points;

  // add goals using RPoints
  final int goalSize = 5;
  final HDrawablePool textPool = new HDrawablePool(points.length);
  textPool
    .autoAddToStage()
    .add(new HEllipse(goalSize)
        .stroke(#FFFFFF, 20)
    ...
    .onCreate(new HCallback() {
          public void run(Object obj) {
            HEllipse ell = (HEllipse)obj;
            int i = textPool.currentIndex();

            println(textPoints[i].x);
            println(textPoints[i].y);
            println();
            ell.loc(width/2+textPoints[i].x, height/2+ textPoints[i].y);
  ...
  swarmPool
    ...
    .onCreate(new HCallback() { 
        public void run(Object obj) {
          HDrawable d = (HDrawable)obj;
          d.loc(random(width), random(2*height/3));
          swarm.addTarget(d);
        }
      }
    );
    
  HTimer trigger = new HTimer(50, swarmSize)
    .callback(new HCallback() { 
        public void run(Object obj)  {
          swarmPool.request();
        }
...
void setup() {
  ...
  grp = RG.getText("Hello", "FreeSans.ttf", 256, CENTER);

  RG.setPolygonizer(RG.UNIFORMLENGTH);
  RG.setPolygonizerLength(20);

  addText(grp.getPoints(), colors.getColor());
}
